...
A stream is a series of `stream_chunk` (assistant text) or `thinking_chunk` (thinking text) frames sharing one `correlation_id`, terminated by exactly one `stream_end` with the same `correlation_id`. In practice the server mints **one `stream_id` per agent turn** (`_run_top_agent` / a sub-agent's turn), not per LLM call: every LLM round of that turn — thinking and text chunks interleaved, plus `toolgen_chunk` tool-argument deltas — shares it, and the single `stream_end` arrives when the turn ends. Clients buffer keyed on that id and segment it themselves where they need per-round text (on a chunk-kind change, `llm.turn_start`, `agent.tool_call_prep` — `kodo-headless` does exactly this, doc/HEADLESS.md §2.1).
...
`ui_text` is a ready-to-render, one-sentence explanation; `reasons` are the matched red-flag/detector codes (`kodo.runtime._engine._watchdog.RedFlag.code`, or a single source-specific code for the mid-stream detectors — extensible, see doc/STUCK_DETECTION.md); `mode` is `"auto"` (autonomous mode, or interactive with `auto_unstuck_interactive`) or `"manual"` (the user clicked "Unstick it"); `source` identifies which detector fired — one of `"stall"`, `"missing_return_result"`, `"cyclic_thinking"`, `"think_in_tool_call"`, `"tool_call_cyclic"`, `"repeated_tool_call"` — and is what kodo-vsix's reducer switches on to decide whether replaying this also needs to flush a live mid-stream buffer (thinking or tool-call-argument display), since only the three mid-stream sources ever fire with one still populated (`"stall"`, `"missing_return_result"` and `"repeated_tool_call"` all fire at a round boundary with nothing buffered, so they need no reducer branch of their own). Also persisted as a `nudge`-*kind* message (`{role: "user" | "assistant", content: <the LLM-visible text>, kind: "nudge", detail: {ui_text, reasons, mode, source}}`) — not a bare marker, since it must round-trip into the live LLM context on resume too — so it replays via `session.history` (§5.11) the same way. Rendered as a non-context, informational feed entry (`exclude_from_context: true`) — the LLM-facing `content` never appears in the feed, only `ui_text`/`reasons`/`mode`/`source`. `role` is `"user"` for every source except `"repeated_tool_call"`, which alone is `"assistant"`. The role tracks user/assistant alternation, not the notice's voice: `"stall"`/`"missing_return_result"` land at a round boundary, and `"cyclic_thinking"`/`"think_in_tool_call"`/`"tool_call_cyclic"` all abort mid-stream with no tool calls parsed — so in every one of those cases the round's own **assistant** message has just been appended and the nudge must be a user turn, while `"repeated_tool_call"` lands directly after a `tool_result` **user** message and must be an assistant one. (`"cyclic_thinking"` and `"tool_call_cyclic"` were `"assistant"` until 2026-09-05; llama.cpp's chat templates rejected the resulting two-assistants-in-a-row with a 400, killing the very turn the nudge was meant to recover. Their `ui_text` is still the first-person sentence it always was — only the LLM-visible `content` became second-person to match the role, so nothing the client renders changed.)
...
### 6.11 `workspace.confirm_folder` — wait for a scaffolded folder to really open

Fired by `GateOrchestrator.fire_confirm_workspace_folder` from `EngineCore._attach_folder`, on the `scaffold_new_project` tool path **only**. The gate that shows nobody anything: it exists purely so the tool call does not return to the agent while VS Code is in the middle of a window reload.

**Why.** `vscode.workspace.updateWorkspaceFolders` restarts the extension host whenever it adds a window's *first* folder, or turns a single-folder window into a multi-root one (`reloadWipesSerializerState(insertAt)`, true for `insertAt <= 1` — see §7.1a). Before this gate the server pushed `workspace.add_folder` (§5.9b) fire-and-forget and `scaffold_new_project` returned immediately, so the agent went straight on to creating files, running commands and reading roots *through* the restart — against a workspace that was being torn down and rebuilt under it. Agents got confused in ways that looked like tool flakiness.

```json
{ "type": "workspace.confirm_folder", "path": "/Users/me/projects/my-app", "name": "my-app" }
```

Same body as `workspace.add_folder`, which it **replaces** on the tool path. The event is still what the `project.create` message (§7, §6.10a) sends: a human clicked "Create Project" and is watching the window, so there is nothing to wait for.

Response payload:

```json
{ "attached": true, "reloaded": true }
```

or

```json
{ "attached": false, "reloaded": false, "error": "VS Code rejected the workspace-folder change" }
```

The client (`confirmWorkspaceFolder` in `src/extension/workspace-attach.ts`) answers `attached: true` only once the directory is genuinely in `vscode.workspace.workspaceFolders`, and re-pushes `workspace.folders` (§5.9c) for this session *before* replying — the server treats the answer as "the workspace is settled" and must not be able to read a stale folder map after hearing it. `reloaded` is informational: it says the attach cost an extension-host restart, which is what explains a 20-second tool call in the server log.

**Surviving the reload needs no special handling.** The request and its future live on the `SessionChannel`, not on the socket (§2.3), so the doomed pre-reload host simply never answers; `replay_pending_requests` re-sends the request verbatim to the window that comes back, which finds the folder already present and answers at once. This is also why the client handler is idempotent by construction — an already-open folder answers immediately and touches nothing — so a request delivered twice (replay after any transient disconnect) is harmless and needs no request-id tracking. The one thing that does *not* survive the reload is the knowledge that a reload happened, so the client arms a small `kodo.reloadedForAttach.<windowId>` `globalState` marker alongside the dead-serializer marker (§7.1a) purely to make `reloaded: true` reportable.

**Timeouts.** The only gate in §6 with a deadline — every other one waits on a human, who may legitimately take an hour, but nothing human is waiting here, so a window that never comes back must not wedge the turn. The server allows `EngineCore.WORKSPACE_ATTACH_TIMEOUT_S` (60s, sized for extension-host restart + tab reconcile + reconnect); the client gives up at 45s (`FOLDER_CHANGE_TIMEOUT_MS`), deliberately first, so the side that actually knows what VS Code did is the one that names the failure. On expiry the server **discards** the pending future (`SessionChannel.discard_response_future`) rather than merely abandoning it — a request left in the pending map would be re-sent to every subsequent reconnect for the rest of the session, with nothing listening for the answer.

**A failure is not an error.** The project exists on disk, is a registered logical root, and is locked to the session regardless of what the window did with it. So the tool returns success with `workspace_attached: false` plus a `warning` telling the agent the user may not see the folder and that it must **not** scaffold it again — never `{"error": ...}`, which would invite a retry and a second directory.

No `pending_prompt`-style state is persisted — same reasoning as §6.8/§6.10.
...
- **Extended 2026-09-13: a remembered workspace that's gone from disk is refused, not opened.** Every window-reloading path — the picker's "Open session and workspace", the manual reconnect button, and Create Project's reconnect-first prompt — funnels through kodo-vsix's `reloadWindowIntoTarget` (`src/extension/session-resume.ts`), which now calls `ensureResumeTargetExists(target)` **before** arming any marker or issuing `vscode.openFolder`/`updateWorkspaceFolders`. A session's remembered folders are never revalidated server-side (the lock rule deliberately keeps a folder in `workspace_folders` forever, §7.1b above), so a bound folder that has since been deleted, renamed or moved is an ordinary state, not a corruption — previously it reloaded the window into a broken/empty workspace with no explanation and then resumed the session into it. The check is **all-or-nothing** across the target's paths (missing paths are reported together in target order; the surviving subset is *not* opened, since that workspace wouldn't match the locked shape anyway and the reconnect would silently come back still-disconnected) and it leaves the remembered shape untouched, so the same reconnect works later if the folder returns. Pure part: `missingResumeTargetPaths(target, exists)` in `workspace-resume-policy.ts`, caller-supplied `exists` (an `fs.statSync(...).isDirectory()` for folders, `fs.existsSync` for a `.code-workspace` file) keeping that module I/O-free like `resumeTarget`'s `codeWorkspaceFileExists`. Note this is a *different* case from §7.1b step 2's silent fallback: a missing `.code-workspace` file still falls back to the folder list at `resumeTarget` time, and only a folder set that is itself missing reaches this error.
