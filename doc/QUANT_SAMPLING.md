...
1. **Reading an attachment.** The whole task specification lives in an attached
   ...
   **pass/fail gate worth the entire run**: a plausible-looking implementation
   ...
   credit.
2. **Building with tests and a toolchain**, whose deliverable is a built
   artifact and whose shape Problem Solver cannot know up front, so it should
   route through `kodo_planner` into `kodo_toolchain_builder` and `kodo_developer` steps.
