...
- `session.jsonl` — append-only LLM context: every message (`role`, `content`) exchanged with the top-level agent, interleaved with subsession start/end markers.
...
A top-level agent (the Guide, or the Problem Solver — they share one agent-agnostic main message history, `_main_messages`) may run for arbitrarily long. After **every** top-level agent turn the engine measures the live context in tokens (the just-finished call's `input + cache_read + cache_write + output` ≈ what the next call will carry) and pushes it to the client as `context.stats` (WS_PROTOCOL.md §5). The limit is the **current model's context window** — the per-model `context_window` in `kodo/llms/_cloud_registry.py` or `kodo/llms/_local_registry.py` (see LLM_REGISTRY.md), resolved via `kodo.llms.get_context_window` for the top-level agent model selected in settings (`ContextCompactor.context_limit`; *not* a global setting — see SETTINGS.md §2.3). Once that measure reaches **90%** of the window the engine compacts **in place** — it does **not** rotate to a new session:
...
2. **Condense.** The engine runs the **`kodo_compactor`** sub-agent (tool-less, single-shot; `subagents/subagent_kodo_compactor.md`) directly, never via `run_subagent` (session titling used to work the same way; it is now `kodo.titling`, a local model with no sub-agent spec at all — see SESSIONS.md). The current `_main_messages` are flattened to a plain-text transcript by `render_transcript` and handed to it as one user message. The call streams nothing to the feed; only its USD cost is folded into the running total. What comes back is **not a briefing** — it is the same transcript, shorter (see "The compaction contract" below).
3. **Mark + reset.** A `compaction` marker — `{type, summary, reason, tokens_before, tokens_after, ts}` — is appended to `session.jsonl`. The live `_main_messages` is then reset to a **single** synthetic user message wrapping that summary, and the gauge is reseeded from a char-based estimate until the next real turn supplies a measured count.
...
**The compaction contract.** The compactor produces a **condensed transcript**, not a briefing — the same conversation, same turns, same order, fewer words. The rules it works to (`subagent_kodo_compactor.md`) are:

- **No fact is ever removed.** Every data point in the input survives into the output — paths, identifiers, line numbers, versions, error text, test names and outcomes, values read from files, answers the user gave. Compaction shortens *how much text it takes to say something*, never *how much is said*. When a passage cannot be shortened without losing a fact, it is copied through unchanged. Notably, this is what now guarantees the old prompt's mandatory "Files changed" section: every file created/edited/moved/deleted stays in the output by exact path, so the resumed agent never re-edits, re-creates, or undoes its own work — it is just carried inline in the turn that did it rather than hoisted into a section.
- **Only four things may go:** duplicates (a file read three times keeps its current content once), superseded state (a pre-edit read collapses into the current state plus what changed), mechanical clutter (call/argument boilerplate, banners, progress lines, decoration, narration), and empty failures (a typo'd path, a transient error that a retry fixed). A failure that *taught* something — a missing dependency, a real assertion failure, a denied permission — is a fact and stays.
- **Turn structure is preserved.** `## USER` blocks are copied **verbatim** and are the only turn boundaries; tool calls are not. Everything between two user prompts collapses into a single `## ASSISTANT` block with the tool results folded in.
- **Thinking is distilled, not dropped.** `[thinking]` blocks are the one place the compactor genuinely rewrites: it keeps every insight, decision, **rejected alternative with its reason**, change of course, and mistake-plus-correction, and discards the wandering and re-derivation. Losing a rejection makes the agent re-propose an idea it already ruled out; losing a correction makes it look like the agent was never wrong, and it repeats the mistake.

**Why the output is self-similar.** A compaction's output becomes the input to the *next* compaction, so the output is deliberately shaped like its input. `render_transcript` labels the leading block `## PRIOR COMPACTED CONTEXT` (matched by `_COMPACTION_PREAMBLE`, which `compaction_context_message` wrote) rather than `## USER`, and the compactor splices it onto the front of the new output and re-condenses it. If it were labelled `## USER`, the verbatim rule would pin the whole prior context word-for-word into every future compaction and the floor would grow without bound.

**Why the headers exist.** `render_transcript` sends three different things as `role="user"`: real user prompts, tool-result envelopes (`_turns.py`/`_resume.py` append these as user messages), and the prior-compaction block. The raw role conflates all three, and the compactor's rules turn on the distinction — so the renderer emits `## USER` / `## TOOL RESULTS` / `## PRIOR COMPACTED CONTEXT` accordingly, and `## USER` means *only* a real user prompt. Anything else wearing that header would be frozen verbatim into every future context.

**Compression ratio is not guaranteed.** Because user prompts are preserved verbatim forever, a session's post-compaction floor is at minimum the sum of all its user prompts. That is normally negligible, but it means the "one pass drops far below the threshold" assumption in `maybe_auto_compact` is an expectation, not an invariant — a read-heavy session compresses hard (bulk payloads keep their facts and lose their formatting), a prompt-heavy one less so.

**Subsession gauge (no compaction).** Whichever sub-agent subsession is currently running gets its own read-only gauge, `context.stats`'s `subsession` field (WS_PROTOCOL.md §5.7a) — measured the same way (last subsession turn's `input + cache_read + cache_write + output`) but against *that subsession's own model's* context window, which can differ from the main top-level agent's (`ContextCompactor.note_subsession_context`, called from `_run_agent_turn` whenever `subsession_model_key` is set instead of `track_context`). It is `null` while no subsession is active, and is cleared the instant one ends — whether it finishes on its own (`_close_subsession` → `ContextCompactor.clear_subsession_context`) or the user clicks Stop mid-run (`WorkflowEngine.stop()` → `_abort_active_subsession`, SESSIONS.md's "The active-subsession pointer": cancellation unwinds through `_drive_subsession` before `_close_subsession` ever runs, so `stop()` has to close the subsession out itself). Subsessions are never compacted — there is no subsession analogue of the 90% auto-trigger or `compact.now`, only the readout.

**Model-switch compaction.** The context limit follows the model, so a model switch can cross the threshold instantly. `config.reload` (sent window-globally when the user changes the model) calls `WorkflowEngine.handle_config_changed()` on **every** live session; it compares the new top-level agent model key to the one that last drove the main context (`ContextCompactor._active_model_key`). If the new model's window is **smaller than the live context size**, the engine compacts **first, using the outgoing model** (`ContextCompactor._run_compaction(reason="model_switch", force_model_key=<old>)`) — so the switch only takes effect on a context that fits the new window — then records the new model and re-emits `context.stats`. Funnelled through the worker queue, so it never races a turn.

**Audit & replay.** The full `session.jsonl` is never rewritten: lines before the marker stay as immutable audit history (§5.1) and are still replayed into the client feed by `history_entries` (the WebView shows the whole conversation — before and after every compaction, of which there may be many). What changes is only the *LLM message history*: `HistoryProjector.load_main_messages` rebuilds it from the **latest** `compaction` marker onward (the summary block + every message appended after it), so the pre-compaction transcript is never resent to the model. The **system prompt is untouched** — it is rebuilt fresh on every turn from `AgentRegistry.get()`, so every shared rule block the agent qualifies for (INTERNALS.md) is always present after a compaction; compaction can never strip them.

**Crash safety.** The marker is a single appended line. A crash before it lands → the next resume rebuilds the full (un-compacted) context and re-checks the threshold on the next turn; a crash after it lands → resume rebuilds from the summary. Both states are valid; the engine needs no separate persisted "compacting" flag (the in-memory `ContextCompactor._compacting` bool only gates the live UI and the manual trigger).
...
4. Sub-agent subsessions finish their pending tool calls and any other pending call gets its stand-in result; the results flow back into the resumed main turn; the top-level agent's next turn resumes.
...
3. The engine resets the in-memory conversation (`_main_messages = []`) — the Guide was reasoning about a state that no longer exists.
...
| Context compaction | Runtime (`runtime/_engine/`, `kodo_compactor` sub-agent) | Auto-triggers at 90% of the current model's context window (or manual `compact.now`, or a switch to a smaller-window model); summarises in place via a `compaction` marker; surfaces `context.stats` / `context.compacting` / `context.compacted` (§4.5). |
