...
| Guide session | The session log of the Guide sub-agent. Persists across the project's lifetime; compacted into a fresh session when context-window usage approaches exhaustion. |
...
- **FR-ORCH-05.** When the Guide session approaches context-window exhaustion, the engine SHALL trigger compaction: summarize the current session into a compact prior-context block, start a fresh Guide session whose initial messages are `{system prompt + compacted summary + fresh index snapshot}`, and surface the transition to the user via the wire (WS_PROTOCOL.md §5). The compaction threshold is a design-level constant, not a requirement.
