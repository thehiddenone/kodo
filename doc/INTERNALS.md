...
sharing the `base_toolchain.md` + `base_dependencies.md` contracts, §11) generates five
...
and `toolchain_cpp`, both `bases: [toolchain, dependencies]`) and its *reader*
...
distinguishing on-demand specialists (e.g. `toolchain_python`, `toolchain_cpp`) from ordered-pipeline
...
**+ `toolchain_python` + `toolchain_cpp`**) and **`guide`** (the full pipeline +
`toolchain_python` + `toolchain_cpp` + the shared `investigator`; its
...
(code+tests), and `toolchain_python`/`toolchain_cpp` (build setup) — via `run_subagent`
