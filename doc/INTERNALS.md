...
**+ `toolchain_python`**) and **`guide`** (the full pipeline + `toolchain_python`
+ the shared `investigator`; its `## Subagents` section embeds the placeholder
and a thin stage→agent map replaces the old hand-written `### Sub-Agent Names`
table).
...
(code+tests), and `toolchain_python` (build setup) — via `run_subagent`
