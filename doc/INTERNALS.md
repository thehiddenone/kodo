...
| `get_root_paths`, `find_files`, `find_text_in_files` | ✅ implemented (workspace search). `get_root_paths` returns the mode-aware root list (bound project in Guided; every workspace folder in Problem Solver) from `ToolContext.root_paths` — unlike almost every other native tool it does **not** set `requires_project` (2026-07-21b): with no workspace bound yet it just returns an empty `roots` list rather than `NO_PROJECT_ERROR`, itself the signal to call `create_new_project` first (`temporary: true` never needed a project either, keyed only by session id). `find_files`/`find_text_in_files` resolve `root` through the active resolver then shell out to the bundled `fd`/`rg` (§10a) via `ToolContext.util_paths`. Granted to `guide` + `problem_solver` + the shared `investigator` + the Problem Solver's `developer`. |
...
language-specific toolchain-setup sub-agent (`toolchain_python`, `toolchain_cpp`,
`toolchain_rust`, sharing the `base_toolchain.md` + `base_dependencies.md` contracts, §11) generates five
...
the `DEPENDENCIES.md` format spec) shared by its *writers* (`toolchain_python`,
`toolchain_cpp`, and `toolchain_rust`, all `bases: [toolchain, dependencies]`) and its *reader*
(`toolchain_depsmgr`, `bases: [dependencies]`).
...
distinguishing on-demand specialists (e.g. `toolchain_python`, `toolchain_cpp`, `toolchain_rust`) from ordered-pipeline
...
**+ `toolchain_python` + `toolchain_cpp` + `toolchain_rust`**) and **`guide`** (the full pipeline +
`toolchain_python` + `toolchain_cpp` + `toolchain_rust` + the shared `investigator`; its
...
|---|---|---|
| `guide` | guided_dev_status, get_root_paths, find_files, find_text_in_files, run_subagent, run_author_critic_iteration, ask_user, rollback, finalize_project, disable_autonomous_mode, **create_new_project**, **init_project** | Arbiter for the **guided** workflow. Resolved through the same `tools_for_agent` path as every other agent. `subagents:` allow-list includes the pipeline agents **+ `toolchain_python` + `toolchain_cpp` + `toolchain_rust` + the shared `investigator`** (preliminary narrative investigation before stage 1 — user-consented in interactive mode, default in autonomous — and mid-pipeline web-opinion consults on substantive ambiguities; findings fold into `narrative_author`'s `instructions` as attributed candidates). |
| `problem_solver` | filesystem, **read_file**, edit_file, create_file, create_directory, run_command, get_root_paths, find_files, find_text_in_files, **toolchain_build/deps**, **run_subagent**, ask_user, **create_new_project**, **init_project** | **Orchestrator** of the **problem-solving** workflow — runs *outside* the Guide pipeline, talking to the user directly and editing real files on disk (see §15). Decides which combination of its sub-agents solves a problem: `investigator` (read-only research), `planner` (scope→plan), `developer` (code+tests), plus `toolchain_python`/`toolchain_cpp`/`toolchain_rust`. Keeps its own direct tools, but delegates the heavy lifting (investigation → Investigator, building → Developer, scoping → Planner) and uses direct tools only to decide the next step, for **trivial retrieval** (a single `read_file`/`find_files`/`find_text_in_files` call answers the question — no cross-source synthesis needed, so the Investigator round-trip buys nothing), or for a trivial ask; it owns writing documentation deliverables from an Investigator `report`. **Embeds `{PLACEHOLDER:SUBAGENTS}` in a `## Subagents` section** — a live caller of the roster mechanism. |
...
| `toolchain_python` | run_command, filesystem, edit_file, create_file, create_directory, find_files, find_text_in_files, get_root_paths, ask_user | **Toolchain-setup** agent (`bases: [toolchain, dependencies]`) for **Python**. Spawnable by `guide` and `problem_solver` (both entry agents — never by a sub-agent, to avoid nested subsessions). Bootstraps/converts a project: generates the five per-platform build scripts (`scripts/{build,format,static_analysis,test,full_build}.{sh,ps1}`) + a `DEVELOPMENT.md` (build/check/test how-to) + a `DEPENDENCIES.md` (the dependency contract), now actually **executed** by `toolchain_build`/`toolchain_deps` (§8). Suggest-then-confirm invocation. |
| `toolchain_cpp` | run_command, filesystem, edit_file, create_file, create_directory, find_files, find_text_in_files, get_root_paths, ask_user | **Toolchain-setup** agent (`bases: [toolchain, dependencies]`) for **C++** — CMake (+ CMakePresets.json) for building, vcpkg manifest mode for dependencies, GoogleTest/CTest for tests, clang-tidy + cppcheck + warnings-as-errors for `static_analysis`. Same spawnability, same five scripts + `DEVELOPMENT.md`/`DEPENDENCIES.md` outputs, and the same suggest-then-confirm invocation as `toolchain_python`. Because vcpkg's manifest mode has no CLI for every dependency operation (no `remove`, no per-package version pin), its `DEPENDENCIES.md` uses the manifest-edit form of `## Operations` that `base_dependencies.md` and `toolchain_depsmgr` now support generally (not just for the `build` kind). |
| `toolchain_rust` | run_command, filesystem, edit_file, create_file, create_directory, find_files, find_text_in_files, get_root_paths, ask_user | **Toolchain-setup** agent (`bases: [toolchain, dependencies]`) for **Rust** — Cargo for building/testing/dependencies, rustfmt for `format`, clippy for `static_analysis`. Same spawnability and five scripts + `DEVELOPMENT.md`/`DEPENDENCIES.md` outputs as its siblings, but with two deliberate departures from `toolchain_cpp`'s stricter model (a user decision made when this agent was added): `build` stays **lenient** (Rust culture treats compiler warnings as advisory locally), and all warnings-as-errors strictness lives in `static_analysis` alone (`cargo fmt --check` + `cargo clippy -- -D warnings`, the latter promoting both clippy lints and plain rustc warnings to failures). Defaults a bootstrapped `Cargo.toml` to `edition = "2021"`. Cargo has a real CLI verb for every dependency operation on every kind except `optional` (which needs a manifest edit to wire the `[features]` entry after `cargo add --optional`), so its `DEPENDENCIES.md` needs far less manifest-edit fallback than `toolchain_cpp`'s. |
...
| `architect`, `requirements_author`, `functional_designer`, `e2e_test_designer`, `test_designer` | filesystem, edit_file, create_file, create_directory, read_file, escalate_blocker | Authors (paired with a critic); `coder` and `e2e_test_coder` additionally hold `toolchain_build`/`toolchain_deps`. |
...
| [_subagents.py](../src/kodo/runtime/_engine/_subagents.py) | mixin | `SubagentMixin` — `_run_subagent`/`_spawn_subagent` + the `_assert_can_spawn` gate, subsession lifecycle/replay, `_run_dependency_manager`, `_run_web_search_agent` (drives the `web_search` agent via `_run_silent_tool_loop_turn`, doc/WEB_SEARCH.md), `_run_author_critic_iteration`. |
...
`investigator` (read-only research), `planner` (scope→plan), `developer`
(code+tests), and `toolchain_python`/`toolchain_cpp`/`toolchain_rust` (build setup) — via `run_subagent`
...
| `init_project` | ✅ Implemented and dispatched (guide + problem_solver); augments an *existing* directory with `.kodo/`+checkpoint mirror, laying out `specs/`/`src/`/`test/` only when it's empty, and adds it to the workspace if not already there |
