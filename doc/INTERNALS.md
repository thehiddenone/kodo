...
| `web_search` | ⚠️ **placeholder** (`WebSearchTool`). Dispatchable, but not yet wired to a provider — returns `{results: [], note}` with an unwired notice; DuckDuckGo public-API wiring is a deferred follow-up. Granted only to the Problem Solver's `investigator` (read-only external research). Security impact `LOW`; available in autonomous mode. |
