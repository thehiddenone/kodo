...
`kodo.agents.subagents.specs._shapes`, since the tool that used to duplicate it
...
- **T0 — leaf packages** (`common`, `project`, `guided_state`, `state`,
  ...
  the user-installed agent store under `~/.kodo/agents` (doc/USER_AGENTS.md),
...
four of them hold (`kodo_guide`, `kodo_problem_solver`, `kodo_narrative_author`,
`kodo_toolchain_builder`), duplicating half of what the spec already said and
...
single, language-agnostic toolchain-setup sub-agent (`kodo_toolchain_builder`, which
carries the five-script contract in its own body and shares `shared_dependencies.md`
with `kodo_toolchain_depsmgr`, §11) generates five
...
`kodo_toolchain_depsmgr` sub-agent, which **executes `DEPENDENCIES.md`** for a single
...
| Module | Defines | Links |
|---|---|---|
| [_installer.py](../src/kodo/llms/llamacpp/_installer.py) | `LlamaInstall`, `install/uninstall/update_llamacpp`, `check_llamacpp_update`, `build_exists`, `fetch_latest_build_number`, `find_installed`, `server_executable` | Platform-aware llama.cpp binary install into `~/.kodo/llama.cpp/bN/`. `fetch_latest_build_number` scans the paginated `GET /releases` listing from `ggml-org/llama.cpp` and returns **the highest `bNNNN` tag whose assets include everything this platform needs to install** (`_required_asset_names` — the platform binary, plus the CUDA runtime zip on Windows — the same set `build_exists` HEAD-probes, so "newest installable" and "this pinned build is installable" cannot disagree). It deliberately **does not use `GET /releases/latest`**: every `bNNNN` nightly is published with `prerelease: true` and that endpoint skips prereleases, so it can only ever resolve to ggml-org's occasional semver wrapper release (e.g. `v0.4.0`, see <https://github.com/ggml-org/ggml/discussions/1579>), and the wrapper's `nightly-tag.txt` asset is **written once when the wrapper is cut and never refreshed** — a snapshot, not a pointer. Following it is what pinned kodo's reported "latest" to b10809 for the nine days after 2026-09-04 while b10941 shipped (fixed 2026-09-13). Builds with missing assets are skipped, not just empty ones: a nightly can exist as a tag while its CI build failed or is still uploading, and per-platform assets appear independently (observed live: b10931 had zero assets between neighbours with 27 each). Scans at most `_RELEASES_MAX_PAGES` (3) pages of 100, raising `RuntimeError` if no installable build is found — a hard failure surfaced to the user, never a silent fall back to a stale number. `install_llamacpp`/`update_llamacpp` take an optional `version` (a build number) to pin an explicit release instead of latest. `build_exists` HEAD-probes a given build's release assets — `server/_app.py`'s `llamacpp.update` handler calls it for a pinned `version` *before* touching the current build, so a nonexistent build number fails without touching the existing install. **`update_llamacpp` installs first and deletes the superseded build afterwards** (each build has its own `bN` directory, so the new one never needs the old one's space): a failed download leaves the previous build working, `llama-meta.json` only moves once the new build has passed `--version`, and the delete becomes pure cleanup whose failure costs a log line rather than the update. **It never reinstalls a build in place** — targeting the installed build falls through `install_llamacpp`'s already-installed early return with no superseded directory to clean up, so nothing is deleted and nothing re-downloaded; the `llamacpp.update` handler short-circuits that case earlier still (§10c). A genuine reinstall is only ever the user's explicit Uninstall-then-Install pair. Directory removal goes through `_rmtree_retrying`, which retries with backoff and clears read-only bits — Windows releases a stopped process's mapped image handles asynchronously, so a delete issued right after stopping llama-server can still hit a sharing violation for a moment. `find_installed` verifies the executable named by `llama-meta.json` still exists and reports "not installed" when it doesn't, so a half-deleted build self-heals via a plain Install instead of stranding every caller on a build that cannot run. No `kodo` imports. |
...
| Module | Defines | Links |
|---|---|---|
| [_loader.py](../src/kodo/agents/_loader.py) | `SubAgent` (frozen: `name`, `tools: frozenset[str]`, `system_prompt`, `source_path`, `capability`, `display_name`, `subagents`, **`subagent_order: tuple[str, ...]`**, **`purpose`**, **`role`**, **`critic`**, **`standalone: bool`**), `AgentLoadError`, `load_agent()` | Parses `subagent_<name>.md` frontmatter + body. Extracts the **`## Purpose`** body section, which becomes the agent's `run_subagent_<name>` tool description (hence caller-agnostic, third person); reads the `role`/`critic`/`standalone` frontmatter; keeps the `subagents:` allow-list in declaration order as `subagent_order`. No `bases:`/`callouts:` — shared text is included by `{SHARED:<name>}` in the body instead. |
| [_subagentspec.py](../src/kodo/agents/_subagentspec.py) + [specs/](../src/kodo/agents/subagents/specs/) | `SubAgentSpec` (frozen: `name`, `input_schema`, `output_schema`, `produces`, `consumes`, `component_paths`, `notes` — **no `description`**; the prose is the agent's `## Purpose`) + one **`specs/<name>.json`** file per agent, loaded at import into `ALL_SUBAGENTS` | The typed input/output contract of a sub-agent — "a tool with agentic behavior". Every sub-agent **except** the top-level agents (`kodo_guide`, `kodo_problem_solver`) has one. The catalog is **data, not Python**: `specs/_loader.py` builds each spec from its JSON (schemas declared as a *shape* — a builder name plus arguments — or `{"shape": "raw", …}` for a literal one), `specs/_shapes.py` holds those declarative builders (`pipeline_input`/`author_output`/`critic_output`), and `specs/_order.py` derives the catalog order from the `produces`/`consumes` graph instead of a hand-kept tuple. `notes` is engineer-facing rationale and never reaches a model. |
| [_registry.py](../src/kodo/agents/_registry.py) | `AgentRegistry`, `shared_token()`, `SHARED_FILE_PREFIX` | Loads all `subagent_*.md` / `agent_*.md` plus every `shared_*.md` block. **Expands** each agent's `{SHARED:<name>}` tokens in one pass — the only prompt-assembly step there is. **Validates** at construction: `tools:` resolve against `ALL_TOOLS`; every `{SHARED:…}` name exists; `working_rules` + `security` are present; a `modifies_files` grant implies `{SHARED:editing}`; `{SHARED:task_input}` implies a `SubAgentSpec`; a shared file contains no token of its own; every `subagents:` entry resolves and (unless a critic) carries a `## Purpose`. Filters `autonomous_mode == "unavailable"` tools when `autonomous=True`, and **auto-grants `return_result`** to any agent with a `SubAgentSpec` (`SUBAGENT_SPECS_BY_NAME`, `spec_for()`). It does **not** describe tools in the prompt (that is the `tools` argument's job, §6) — including sub-agents: `run_subagent_specs()` builds each callee's tool from that callee's own `## Purpose`. |
...
roughly half never write a file (every critic, `kodo_investigator`, `kodo_compactor`,
`kodo_planner`, `kodo_web_search`), and telling a critic how to keep a diff minimal
contradicts the "use only your granted tools" rule in `{SHARED:security}`.
`{SHARED:callouts}` reaches only `kodo_guide` and `kodo_problem_solver`: a sub-agent's
...
`kodo_compactor` included, even though `_generate_compaction_summary` seeds that one
with a bare `"Conversation transcript to compact: …"` message rather than a
rendered `# Task` + `## Input Parameters` turn. `kodo_compactor` therefore includes
...
format spec, shared by its *writer* (`kodo_toolchain_builder`) and its *reader*
(`kodo_toolchain_depsmgr`). `shared_escalation.md` is when and how to hand a blocker
...
`kodo_toolchain_builder` it had a single consumer and was folded into that agent's
body.)
...
top-level agent running `list[Message]`, agent-agnostic across Guide/Problem
...
| [_llm.py](../src/kodo/runtime/_engine/_llm.py) | mixin | `LLMPlumbingMixin` — `_resolve_plugin`/`_resolve_model_key`, `_run_silent_return_turn`, `_run_silent_tool_loop_turn` (a silent, multi-round, non-subsession tool-calling turn for the `kodo_web_search` agent — deadline- and round-capped, doc/WEB_SEARCH.md), `_security_judge`. |
| [_turns.py](../src/kodo/runtime/_engine/_turns.py) | mixin | `TurnLoopMixin` — `_run_top_agent` (+ no per-agent wrappers), `_run_agent_turn` (the generic LLM tool loop), `_dispatch_tool_calls` → `_dispatch_one_tool_call` (one call end to end; split out purely so the batch loop can wrap each call in its own failure boundary — STATE_AND_LIFECYCLE.md §10.1), `_finalize_tool_result`, `_make_dispatcher`, attachment storing. |
| [_subagents.py](../src/kodo/runtime/_engine/_subagents.py) | mixin | `SubagentMixin` — `_run_subagent`/`_spawn_subagent` + the `_assert_can_spawn` gate, subsession lifecycle/replay, `_run_dependency_manager`, `_run_web_search_agent` (drives the `kodo_web_search` agent via `_run_silent_tool_loop_turn`, doc/WEB_SEARCH.md), `_run_review_loop`/`_run_review_round`/`_record_findings`, plus the findings scope threading (`_findings_dir`/`_findings_snapshot`/`_document_status`, doc/FINDINGS.md) and the plan hook (`_is_planner`/`_initialize_plan`/`_plan_dir`, doc/PLANNING.md — reads a `planner: true` callee's result after whichever flow it declared, and lets `PlanConflictError` propagate). |
...
| [_checkpointing.py](../src/kodo/runtime/_engine/_checkpointing.py) | collaborator | `CheckpointCoordinator` (+ `CheckpointHost`) — `_MUTATING_TOOLS`, prepare/commit around mutating dispatches, `record_guided_revision`, owns the `RootMirrorManager` (`.mirrors`). Neither `prepare` nor `commit` ever raises: both degrade to "no checkpoint for this call" on `_CHECKPOINT_FAILURES` and log (CHECKPOINTS.md §10). |
...
- **Public client entry points** (registered as WS handlers in `_app`, §14):
  ...
  sets the workflow (`SessionState.top_agent`, normalised to `"guided"` |
...
- `_run_worker()` — dequeues one task at a time. **First it freezes the
  ...
  **routes by `top_agent`**: `"problem_solving"` →
  `_run_problem_solver_with_input` (if the `kodo_problem_solver` agent is present,
  else `_handle_input_no_agent`); `"judge"` → `_run_judge_with_input` likewise
  gated on `kodo_judge` agent availability (validator-only — see §15); otherwise →
...
- `_run_guide_with_input` / `_run_problem_solver_with_input` /
  ...
  `_run_top_agent(agent_name, text, attachments)`.
- `_run_top_agent(agent_name, ...)` → the shared top-level agent driver: loads
  ...
  autonomous mode for both top-level agents),
  `stop_after_tools = lambda: dispatcher.stop_requested`,
  `persist = _persist_main_messages(agent_name)` and
  `flush_before_dispatch=True`. The two top-level agents differ only in system
...
- `_run_dependency_manager` (exposed via `_EngineServices.run_dependency_manager`,
  ...
  `kodo_toolchain_depsmgr` agent straight through `_spawn_subagent` (the ungated
...
`mode.set` → `handle_mode_set` (Autonomous/Interactive), `agent.set` →
...
`checkpoint.rollback` → `_handle_checkpoint_rollback`**) — `mode.set`/`agent.set`
...
**Prompt → work (problem-solving):** when `top_agent == "problem_solver"`,
the worker routes the same prompt to `_run_problem_solver_with_input` instead.
The `kodo_problem_solver` agent runs `_run_agent_turn` with its own dispatcher,
...
`kodo_planner` (investigates the codebase, then returns `codebase_context` + an
ordered plan; the route for a code change whose shape it doesn't already
know), `kodo_investigator` (web research and documentation reports), `kodo_developer`
(code+tests), and `kodo_toolchain_builder` (build setup, any language) — via `run_subagent`
...
**Prompt → work (judge):** when `top_agent == "judge"`, the worker routes
to `_run_judge_with_input` instead. The `kodo_judge` agent (`agent_kodo_judge.md`) runs
...
tool `kodo_problem_solver`/`kodo_toolchain_builder` runs use to execute a project's
...
(doc/VALIDATOR.md §9.2); `agent_kodo_judge.md`'s own prose gates it to RVP-directed
...
`toggle_top_agent` → `agent.set {name}` → `handle_agent_set` sets
`SessionState.top_agent`, which the worker reads at the next dequeue to pick
the top-level agent. Both emit `EVT_STATE`; the Guide can also drop autonomous
...
| Toolchain agent tools (`toolchain_build`/`toolchain_deps`) | ✅ Implemented. `toolchain_build` executes the toolchain-setup agent's generated `scripts/<step>` (§8). `toolchain_deps` spawns the `kodo_toolchain_depsmgr` sub-agent (via the ungated `run_dependency_manager` service) to execute the project's `DEPENDENCIES.md` for one add/remove/update op; a missing `DEPENDENCIES.md` comes back as a remediation message pointing at the toolchain-setup agent (§8). |
...
| Native file-IO / `run_command` / `read_file` tools | ✅ Implemented; granted to authoring sub-agents and `kodo_problem_solver` |
| Two user-facing workflows (`guided` Guide / `problem_solving` Problem Solver), plus the validator-only `kodo_judge` (Judge, almost entirely read-only plus one scoped `toolchain_build` exception — never sent by kodo-vsix) | ✅ Implemented; selected by `agent.set` → `SessionState.top_agent` |
