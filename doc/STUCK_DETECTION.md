...
- **`scope`**: `"top_level" | "top_level_and_subagents"` (default `"top_level"`) — whether only the shared entry-agent turn (Guide/Problem Solver) is watched, or sub-agent turns (spawned through a `run_subagent_<name>` tool) too.
...
- **Entry-agent scope**: a **dedicated** streak, `WatchdogMixin._cycle_streak` (declared alongside `_stuck_streak` in `_core.py`/`_proto.py`/`_watchdog.py`), deliberately separate from `_stuck_streak` so an ordinary stall and a detected thinking loop never combine to trip either escalation's two-strike cap. Cleared under the same "genuine progress" conditions as `_stuck_streak` — the *existing* clearing hooks (`_on_stall`'s no-flags branch, `_make_progress_handler`'s `on_tool_calls` closure) were **extended** to also clear `_cycle_streak`, not duplicated. First hit: `_persist_cyclic_thinking_notice` persists a single artifact — one message that is *both* the LLM-visible course-correction the agent reads back next round *and* (via `kind="cyclic_thinking_notice"`, a genuinely new kind, not a reuse of `"stopped_notice"`) the `<kodo_crit>` callout the user sees — mirroring `_persist_nudge`'s dual role; `retry=True`. Second consecutive hit: `_persist_cyclic_thinking_critical` — client-only, never fed back to the LLM, mirroring `_persist_stuck_critical` but with distinct wording naming the actual cause; `retry=False`, ending the turn (the session settles into `awaiting_user`, the existing idle-equivalent phase — no new phase was needed).
...
**Persistence & rendering** — two new, distinct wire events, `EVT_AGENT_CYCLIC_THINKING_NOTICE`/`agent.cyclic_thinking_notice` and `EVT_AGENT_CYCLIC_THINKING_CRITICAL`/`agent.cyclic_thinking_critical` (`EngineEmitters.emit_cyclic_thinking_notice`/`emit_cyclic_thinking_critical`), kept separate from `EVT_AGENT_UNSTUCK_NUDGE`/`EVT_AGENT_STUCK_CRITICAL` since the root cause and message differ. The notice persists as a `kind="cyclic_thinking_notice"` message (`HistoryProjector._message_to_entries` passes its actual persisted content through, unlike `"stopped_notice"`'s hardcoded client-side string, so the wording stays single-sourced in `_CYCLIC_THINKING_NOTICE`); the critical hit persists as an `agent_cyclic_thinking_critical` marker (`HistoryProjector._marker_to_entries`), exactly like `agent_stuck_critical`. kodo-vsix renders both as a red `<kodo_crit>` callout (`SessionEntryView.tsx`) — but the two live reducer cases are **not** a plain append like `agent_unstuck_nudge`/`agent_stuck_critical`: the notice fires mid-round, with the round's repeated thinking text still live in `state.streamingThinking` (every fragment was forwarded to the client before the detector saw it), and round 2 starts immediately after with no intervening `stream_end` — a plain append would leave round 1's garbage sitting in the buffer for round 2's genuine `thinking_token` events to append onto. The notice case therefore commits+clears `streamingThinking`/`streamingTokens`/`thinkingActive`/`thinkingStartedAt` (mirroring `toolgen_token`'s "starting" branch, which has the same "more streaming still coming" shape) but deliberately leaves `awaitingLlm`/`streaming`/`llmWaiting` alone, since round 2's imminent `llm_turn_start` will set those correctly. The critical case, by contrast, *does* fully mirror `interrupted`/`runtime_error` (clearing every waiting indicator too) since the turn genuinely ends there.
