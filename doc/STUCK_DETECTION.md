...
Before 2026-08-03 this shape existed twice, independently: `_persist_nudge` (stall/missing-`return_result`, always `role="user"`, `kind="agent_unstuck_nudge"`) and `_persist_cyclic_thinking_notice` (always `role="assistant"`, `kind="cyclic_thinking_notice"`). Both carried the identical `{llm_text via content, ui_text via detail}` split — and, worse, kodo-vsix rendered them inconsistently: the cyclic-thinking notice showed as a **red** `<kodo_crit>` callout even though it is a real nudge the model keeps going from, visually indistinguishable from the two truly-terminal "gave up" notices. The two were unified into one `Nudge` type, one persistence path (`WatchdogMixin._persist_nudge(self, *, agent_name, subsession_id, nudge: Nudge, role: str)`), one `kind="nudge"`, and one wire event (`EVT_NUDGE`/`agent.nudge`) — every closure below builds a `Nudge` and calls this one method rather than persisting/emitting anything itself. `role` is still caller-supplied, and it is decided by **what the nudge lands after**, never by the notice's voice. `"user"` for stall/missing-`return_result` (the model responds to it as if told by the user to continue) and for all three mid-stream detectors (§2.7/§2.9/§2.10), each of which aborts with no tool calls parsed — so `_run_agent_turn` has just appended the round's own **assistant** message and a second one in a row is invalid. `"assistant"` for §2.11's repeated-tool-call notice alone, which fires after a `tool_result` **user** message and needs assistant-voice to keep the same alternation intact.

This was got wrong for §2.7 and §2.10 until 2026-09-05: both were persisted `role="assistant"` for voice reasons, which put two assistant messages at the end of the list. llama.cpp's chat templates reject that outright (`400 … "Cannot have 2 or more assistant messages at the end of the list."`), so on local models the nudge that exists to *recover* the turn was reliably what killed it (session 1788649506). §2.9 had always used `"user"` from the identical structural position. The fix keeps the user-visible callout unchanged by splitting each notice into a pair: `ui_text` stays the first-person sentence the feed renders, while `llm_text` — what the model actually reads back — is second-person and user-voice, matching `_MISSING_RETURN_RESULT_LLM_TEXT` and `_think_in_tool_call_llm_text`.
...
**Persistence & rendering** — reuses the unified `Nudge`/`_persist_nudge` (§2.5) verbatim: `source="think_in_tool_call"`, `role="user"` — the role §2.7 and §2.10 now also use, for the alternation reason set out in §2.5; this one had it right from the start. The critical hit (`EVT_AGENT_THINK_IN_TOOL_CALL_CRITICAL`/`agent.think_in_tool_call_critical`, `EngineEmitters.emit_think_in_tool_call_critical`) mirrors `_persist_cyclic_thinking_critical`'s shape exactly — its own marker/event, kept distinct rather than reusing an existing critical type, since the root cause differs.

### 2.10 Mid-stream tool-call-argument repetition detection

The companion check to §2.9, on the same `ToolCallArgDelta` stream: a model's tool-call arguments degenerating into a repetition loop — the same failure mode §2.7 catches inside thinking blocks, but inside structured tool-call data instead. Reuses `kodo.runtime._cyclic_thinking.CyclicThinkingDetector` as a second, independent instance per round, fed `ToolCallArgDelta.text` fragments instead of `ThinkingDelta.text` ones — but built through `CyclicThinkingDetector.for_tool_call_arguments()`, **not** the bare constructor. The *algorithm* has nothing thinking-specific about it; its default *calibration* very much does, and that distinction was originally missed.

**Why this stream needs its own calibration.** The default profile fires once some 24-character block has repeated three times running — sound for prose, where that really is a loop. Tool-call arguments are not prose: they are JSON payloads carrying markdown, source code, tables, padding and indentation, where the identical pattern is ordinary *formatting*. Measured against the shipped default, all of the following trip it inside the first ~150 characters of a `create_file`: any run of ≥72 identical characters (a `---`/`===` rule, or 72 spaces of alignment), a column-aligned markdown table separator whose cells are ≥22 dashes, and three identical template rows of ≥24 characters. In session 1788649506 this killed an architect's turn ~200 characters into a perfectly good architecture document; whether a turn survived came down to whether the model had padded its table columns.

`for_tool_call_arguments()` adds two floors, and changes nothing about the thinking-block profile:

- `_ARGS_MIN_DISTINCT_CHARS = 8` — the repeated block must contain at least this many *distinct* characters. This disqualifies the low-entropy cases outright, at any length, while any repeat of real prose or code clears it within a few words.
- `_ARGS_MIN_BUFFERED_CHARS = 1200` — nothing fires until this many characters of arguments have streamed, so a verdict is never reached on a sample too small to mean anything. Deliberately generous: the detector exists to save a turn from a loop that would otherwise run for minutes, and letting it write ~1200 characters first costs a couple of seconds. Kept below `_MAX_NEEDED_CHARS` so the floor is always cleared before the buffer is trimmed (the counter behind it is a true running total regardless).
...
**Escalation** — `WatchdogMixin._make_tool_call_cyclic_handler`, gated by the exact same `stuck_detection` settings as ordinary stalls and §2.7's thinking-block detector (unlike §2.9: this *is* a heuristic — "this looks like a loop" — not a hard protocol violation, so a user can reasonably turn it off). Otherwise identical in shape to `_make_cyclic_thinking_handler`: own dedicated streak, `WatchdogMixin._tool_call_cycle_streak`; entry-agent one-nudge-then-critical, sub-agent capped-then-silent; remediation always immediate for both strikes and both scopes (the stream is already dead and the repeated content already generated by the time this fires — nothing left to defer or ask about). The notice is a *pair* of texts (mirrors `_CYCLIC_THINKING_UI_TEXT`/`_CYCLIC_THINKING_LLM_TEXT`). The feed shows `_TOOL_CALL_CYCLIC_UI_TEXT`, first-person: *"I noticed my tool call's arguments had fallen into a repetitive loop, generating the same content over and over, and stopped it before it could burn through the rest of the turn. I will not continue down that line — let me regenerate this tool call's arguments from scratch."* The model reads `_TOOL_CALL_CYCLIC_LLM_TEXT`, second-person, because the message is persisted `role="user"` (§2.5): *"Your tool call's arguments fell into a repetitive loop, generating the same content over and over, and the call was stopped before it could burn through the rest of the turn. Do not continue down that line — make the call again, generating its arguments from scratch."*

Both strikes also `_log.warning` the offending argument tail, not just the critical. This is a heuristic that can be wrong, and until it logged here the only record of *what* it fired on was the nudge — which carries the generic notice text, not the content. The argument text itself never becomes a `ToolCallEvent`, so the request log's `tool_call_arg_delta` entries (`kodo.llms._logger._event_to_dict`, added at the same time — it previously fell through to the bare `{"type": "ToolCallArgDelta"}` fallback and dropped the text) and this warning are the only two places it survives at all.

**Persistence & rendering** — reuses the unified `Nudge`/`_persist_nudge` (§2.5): `source="tool_call_cyclic"`, `role="user"` (same alternation rule and same first-person-`ui_text`/second-person-`llm_text` split as the cyclic-thinking notice). The critical hit (`EVT_AGENT_TOOL_CALL_CYCLIC_CRITICAL`/`agent.tool_call_cyclic_critical`, `EngineEmitters.emit_tool_call_cyclic_critical`) mirrors the other critical emitters — its own marker/event, kept distinct since the root cause differs.
...
**Persistence & rendering** — reuses the unified `Nudge`/`_persist_nudge` (§2.5): `source="repeated_tool_call"`, `role="assistant"` — the one notice in this document that really is persisted assistant-voice. `role="assistant"` also keeps strict user/assistant alternation here, which matters at this position specifically: the nudge lands right after the round's `tool_result` **user** message, so a `"user"` nudge would put two user messages back to back.
