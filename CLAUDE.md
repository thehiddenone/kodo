...
- `filesystem`, `edit_file`, `read_file` — `ToolSpec`s in [src/kodo/toolspecs/](src/kodo/toolspecs/), dispatched in-process by their `Tool` subclasses in [src/kodo/tools/](src/kodo/tools/) (`FilesystemTool`, `EditFileTool`, `ReadFileTool`).
...
- __Completion__ — production is a `filesystem`/`edit_file` call; acceptance is driven entirely by the engine after a critic's `document_feedback(accept: true)` call — no sub-agent reports completion itself.
