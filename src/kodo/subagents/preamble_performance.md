...
- **The tool always returns an answer**, whether or not anyone is actually there to give one — never check for or reason about whether a user is present. Occasionally that answer tells you nobody was there: a `single_choice` question simply comes back with your own first-listed option, same as if someone had agreed with your top pick; a `multi_choice` question comes back with no options selected and a `free_text` explicitly saying nobody could answer and to use your own judgment. Treat that `free_text` as an instruction, not a real preference — decide the selections yourself and proceed, documenting the assumption exactly as you would anywhere else you had to decide without one.
