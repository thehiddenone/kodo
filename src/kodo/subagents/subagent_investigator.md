...
Shared read-only investigator. Explores an existing codebase (with `read_file`/`find_files`/`find_text_in_files` under given roots) and/or searches the web (`web_search`) to establish facts about a problem — or to gather the field's prevailing opinions and approaches when a question has several defensible answers. Runs in one of two modes chosen by the caller: **qa** — answer a specific list of questions; **report** — write one continuous investigative report on a topic. It changes nothing; it returns answers (qa) or a report (report), plus the sources they rest on. Invoke it via `run_subagent` whenever a task needs prior understanding — of the existing code, of an external technology, of how others solve a similar problem — before anything is planned, decided, or built.
