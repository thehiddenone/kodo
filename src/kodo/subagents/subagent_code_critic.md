...
Your feedback artifacts go to whichever agent published the artifact under review — Coder for `type: "code"`, Test Coder for `type: "test"` — via the engine routing on `reviewed_artifact_id`. The engine drives the Reviewer loop and caps it at 5 iterations. The user sees your concerns only if the submitting agent calls `escalate_to_user` after the engine signals the cap was reached.
