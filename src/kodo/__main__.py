...
``--tools LLM_ID AGENT`` prints that other half: the exact ``tools=[...]``
payload the OpenAI-compatible client sends for that agent, built by
...
across every cloud vendor. Today it only has to *resolve* for either command —
the rendered prompt is identical for every model, because no plugin appends
anything model-specific (``LlamaPlugin`` and ``AnthropicPlugin`` both send the
agent's system prompt as-is), and the OpenAI tools shape is the one
``LlamaPlugin`` builds regardless of which vendor ``LLM_ID`` resolves to. The
argument is kept because per-LLM variation is planned: when model-specific
behavior is added, this is where it will show up, and the invocation will not
have to change.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return _run_tools(args.llm_id, args.agent)
    except BrokenPipeError:
        # A downstream reader closed the pipe (`| head`, quitting a pager). The
        # output these commands print can run to ~100 KB, so piping is the
        # normal way to use them — not a failure worth a traceback. Point
        # stdout at devnull so the interpreter's shutdown flush cannot raise a
        # second time and print one anyway (the recipe from the Python docs'
        # note on SIGPIPE).
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    group.add_argument(
        ...
        "OpenAI-compatible client, for LLM_ID — a local registry name or a "
        "cloud model_id. Both are accepted and must resolve; the payload "
        "shape itself is currently the same for every model.",
    )
    parsed = parser.parse_args(argv)
    parsed.llm_id, parsed.agent = parsed.system_prompt or parsed.tools
    return parsed
...
def _resolve(llm_id: str, agent_name: str) -> tuple[AgentRegistry | None, SubAgent | None, int]:
    """Resolve *agent_name* then *llm_id*, the shared validation both commands need.
    ...
    Returns:
        ...
        checked before the LLM id. The registry rides along because ``--tools``
...
def _run_system_prompt(llm_id: str, agent_name: str) -> int:
    ...
    """
    registry, agent, code = _resolve(llm_id, agent_name)
...
def _run_tools(llm_id: str, agent_name: str) -> int:
    """Resolve *llm_id* + *agent_name* and print the agent's OpenAI tools payload.
