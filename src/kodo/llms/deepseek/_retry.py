"""Exponential-backoff retry wrapper for DeepSeek API calls (2s / 8s / 32s).

Thin wiring over the shared :mod:`kodo.llms._provider_retry` core — see that
module for the actual retry/backoff/classification algorithm. DeepSeek is
...
Alibaba plugins' own ``_retry.py`` handle — this module mirrors those exactly.
"""
