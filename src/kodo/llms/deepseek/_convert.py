"""Message/tool conversion for DeepSeek's OpenAI-compatible endpoint.

DeepSeek's API speaks Chat Completions and is OpenAI-SDK-compatible
(https://api-docs.deepseek.com/) — same wire shape as
:mod:`kodo.llms.google._convert`/:mod:`kodo.llms.alibaba._convert`, and this
module is adapted from the latter (itself adapted from Google's, itself
adapted from the local :mod:`kodo.llms.llamacpp._llama` plugin). Deliberately
a separate copy, not an import of any of them — see
...
other vendor here except Gemini. No DeepSeek documentation was found
...
3 of the now-5 vendors in this codebase, flagged here as an assumption to
revisit if DeepSeek's docs turn up a stricter requirement later.

There is no cache-breakpoint logic here: DeepSeek's own context caching is
automatic and disk-backed (https://api-docs.deepseek.com/guides/kv_cache),
