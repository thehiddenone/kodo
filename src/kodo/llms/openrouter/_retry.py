"""Exponential-backoff retry wrapper for OpenRouter API calls (2s / 8s / 32s).

Thin wiring over the shared :mod:`kodo.llms._provider_retry` core — see that
module for the actual retry/backoff/classification algorithm. OpenRouter is
reached through the ``openai`` Python SDK pointed at a custom ``base_url``
(OpenRouter's own OpenAI-compatible endpoint), so it raises the same
``openai.*`` exception classes the OpenAI/Meta/Google/Alibaba/DeepSeek/Kimi
plugins' own ``_retry.py`` handle — this module mirrors those exactly.
"""
