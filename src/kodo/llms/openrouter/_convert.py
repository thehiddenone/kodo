"""Message/tool conversion for OpenRouter's OpenAI-compatible endpoint.

OpenRouter speaks Chat Completions and is OpenAI-SDK-compatible
(https://openrouter.ai/docs/quickstart) — same wire shape as
:mod:`kodo.llms.kimi._convert`/:mod:`kodo.llms.deepseek._convert`/
:mod:`kodo.llms.alibaba._convert`, and this module is adapted from Kimi's.
Deliberately a separate copy, not an import of any of them — see
...
other vendor here except Gemini — OpenRouter aggregates arbitrary upstream
models with no single documented ``thought_signature``-style mandatory replay
requirement, so dropping it is the same conservative default the other
Chat-Completions-shaped vendors use.

There is no cache-breakpoint logic here: OpenRouter's own upstream providers
handle prompt caching on their own terms (some automatic, some not), with no
single ``cache_control``-style marker mechanism kodo could target across the
whole catalog — so ``LLMPlugin.stream_query``'s ``cache_breakpoints``
