"""Exponential-backoff retry wrapper for Alibaba Model Studio (DashScope) API calls (2s / 8s / 32s).

Thin wiring over the shared :mod:`kodo.llms._provider_retry` core — see that
module for the actual retry/backoff/classification algorithm. Qwen is reached
through the ``openai`` Python SDK pointed at a custom ``base_url`` (Alibaba
Cloud Model Studio's OpenAI-compatible endpoint,
