"""Exponential-backoff retry wrapper for Google Gemini API calls (2s / 8s / 32s).

Thin wiring over the shared :mod:`kodo.llms._provider_retry` core — see that
module for the actual retry/backoff/classification algorithm. Gemini is
...
so it raises the same ``openai.*`` exception classes the OpenAI/Meta plugins'
own ``_retry.py`` handle — this module mirrors those exactly.
"""
