"""Exponential-backoff retry wrapper for Meta Model API calls (2s / 8s / 32s).

Thin wiring over the shared :mod:`kodo.llms._provider_retry` core — see that
module for the actual retry/backoff/classification algorithm. Meta's Model
API is served through the ``openai`` Python SDK pointed at a custom
``base_url`` (it is documented as drop-in compatible with that SDK), so it
raises the same ``openai.*`` exception classes the OpenAI plugin's own
``_retry.py`` handles — this module mirrors that one exactly.
"""
