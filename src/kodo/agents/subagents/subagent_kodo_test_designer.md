---
name: kodo_test_designer
display_name: Test Designer
critic: kodo_test_design_critic
user_review: true
capability: medium
tools:
  - filesystem
...
Test Designer produces the **Test Plan** for one component: the behavioral test cases that pin the responsibility's requirements, designed against its Functional Design. Call it per component after the design is accepted. Invoke it via `run_subagent_kodo_test_designer`, which runs the whole author/critic loop against `kodo_test_design_critic` — you do not invoke the critic and you do not iterate by hand.
