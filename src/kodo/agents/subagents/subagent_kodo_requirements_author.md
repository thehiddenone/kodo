---
name: kodo_requirements_author
display_name: Requirements Author
critic: kodo_requirements_critic
user_review: true
capability: medium
tools:
  - filesystem
...
Requirements Author turns the accepted architecture into a structured **requirements document**, translating each single responsibility into clear, measurable, testable requirements with stable IDs. Call it after the architecture is accepted. Invoke it via `run_subagent_kodo_requirements_author`, which runs the whole author/critic loop against `kodo_requirements_critic` — you do not invoke the critic and you do not iterate by hand.
