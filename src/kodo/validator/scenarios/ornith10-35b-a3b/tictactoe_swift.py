"""Scenario: CLI Tic-Tac-Toe vs. computer, with tests + toolchain, in Swift.

Selector: ``ornith10-35b-a3b.tictactoe_swift`` (or ``ornith10-35b-a3b`` / ``all``).
...
toolchain triggers fire and it spawns ``toolchain_builder`` for Swift. The
...
SCENARIO = Scenario(
    name="tictactoe-toolchain-swift",
    prompts=[PROMPTS.get("tictactoe_toolchain/task").format(language="Swift")],
    ...
    roots=[RootSpec(name="tictactoe-swift")],
