"""Scenario: CLI Tic-Tac-Toe vs. computer, with tests + toolchain, in Java.

Selector: ``ornith10-35b-a3b.tictactoe_java`` (or ``ornith10-35b-a3b`` / ``all``).
...
toolchain triggers fire and it spawns ``toolchain_builder`` for Java. The
...
SCENARIO = Scenario(
    name="tictactoe-toolchain-java",
    prompts=[PROMPTS.get("tictactoe_toolchain/task").format(language="Java")],
    ...
    roots=[RootSpec(name="tictactoe-java")],
