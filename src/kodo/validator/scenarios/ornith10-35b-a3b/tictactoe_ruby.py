"""Scenario: CLI Tic-Tac-Toe vs. computer, with tests + toolchain, in Ruby.

Selector: ``ornith10-35b-a3b.tictactoe_ruby`` (or ``ornith10-35b-a3b`` / ``all``).
...
toolchain triggers fire and it spawns ``toolchain_builder`` for Ruby. The
...
SCENARIO = Scenario(
    name="tictactoe-toolchain-ruby",
    prompts=[PROMPTS.get("tictactoe_toolchain/task").format(language="Ruby")],
    ...
    roots=[RootSpec(name="tictactoe-ruby")],
