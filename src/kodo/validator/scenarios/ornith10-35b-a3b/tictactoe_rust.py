"""Scenario: CLI Tic-Tac-Toe vs. computer, with tests + toolchain, in Rust.

Selector: ``ornith10-35b-a3b.tictactoe_rust`` (or ``ornith10-35b-a3b`` / ``all``).
...
toolchain triggers fire and it spawns ``toolchain_builder`` for Rust. The
...
SCENARIO = Scenario(
    name="tictactoe-toolchain-rust",
    prompts=[PROMPTS.get("tictactoe_toolchain/task").format(language="Rust")],
    ...
    roots=[RootSpec(name="tictactoe-rust")],
