"""Scenario: CLI Tic-Tac-Toe vs. computer, with tests + toolchain, in TypeScript.

Selector: ``ornith10-35b-a3b.tictactoe_typescript`` (or ``ornith10-35b-a3b`` / ``all``).
...
toolchain triggers fire and it spawns ``toolchain_builder`` for TypeScript. The
...
SCENARIO = Scenario(
    name="tictactoe-toolchain-typescript",
    prompts=[PROMPTS.get("tictactoe_toolchain/task").format(language="TypeScript")],
    ...
    roots=[RootSpec(name="tictactoe-typescript")],
    ...
    # toolchain_builder for TypeScript.
    modes=Modes(
        autonomous=False,
