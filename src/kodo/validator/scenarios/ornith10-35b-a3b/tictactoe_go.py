"""Scenario: CLI Tic-Tac-Toe vs. computer, with tests + toolchain, in Go.

Selector: ``ornith10-35b-a3b.tictactoe_go`` (or ``ornith10-35b-a3b`` / ``all``).
...
toolchain triggers fire and it spawns ``toolchain_builder`` for Go. The
...
SCENARIO = Scenario(
    name="tictactoe-toolchain-go",
    prompts=[PROMPTS.get("tictactoe_toolchain/task").format(language="Go")],
    ...
    roots=[RootSpec(name="tictactoe-go")],
