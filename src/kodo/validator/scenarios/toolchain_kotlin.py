"""Scenario: CLI Fibonacci number calculator, with tests + toolchain, in Kotlin.

Selector: ``toolchain_kotlin`` (or ``all``).
...
spawns ``toolchain_builder`` for Kotlin. This file is content-only (see
...
SCENARIO = Scenario(
    name="toolchain-kotlin",
    prompts=[PROMPTS.get("toolchain/task").format(language="Kotlin")],
    user_proxy_prompt=PROMPTS.get("toolchain/upp"),
    result_validation_prompt=PROMPTS.get("toolchain/rvp"),
    roots=[RootSpec(name="fibonacci-kotlin")],
