"""Scenario: CLI Fibonacci number calculator, with tests + toolchain, in TypeScript.
...
spawns ``toolchain_builder`` for TypeScript. This file is content-only (see
...
SCENARIO = Scenario(
    ...
    prompts=[PROMPTS.get("toolchain/task").format(language="TypeScript")],
    user_proxy_prompt=PROMPTS.get("toolchain/upp"),
    result_validation_prompt=PROMPTS.get("toolchain/rvp"),
    roots=[RootSpec(name="fibonacci-typescript")],
