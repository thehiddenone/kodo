...
3. **Read-only fast path** — every executable on the conservative read-only
   list, with no segment carrying an outside-workspace finding, allows.
   Value expansions (``$VAR``) are tolerated here: an unknown value fed to a
   reader cannot mutate. A read-only executable that *writes* via a
   redirection (``cat > file``, ``echo … > file``) is judged per segment
   instead, in step 4 below — it allows too, as long as the write target is
   workspace-confined (or under the OS temp directory) and statically
   resolvable; only an escaping or unresolvable write target still asks.
3a. **Toolchain-builder scripts** — a segment whose own executable directly
    invokes one of ``toolchain_builder``'s generated
    ``scripts/<step>.{sh,ps1}`` entrypoints under a workspace root (judged
    per segment, via ``._analysis.CommandAnalysis.segment_toolchain_script``)
...
def evaluate_command(
    ...
    *,
    ...
    for i, segment in enumerate(analysis.segments):
        ...
        if analysis.segment_toolchain_script[i]:
            continue  # scripts/<step>.{sh,ps1} generated by toolchain_builder.
...
def _judge_segment(
    ...
    *,
    ...
    if exe == "command":
        if any(_flag_hit(f, segment.flags) for f in ("-v", "-V")):
            # Query mode (`command -v`/`-V foo`): prints where `foo` would
            # resolve, executes nothing — the POSIX `which`/`type` equivalent.
            return ok
        # Plain `command foo [args…]` transparently executes `foo`, bypassing
        # any shell function/alias of the same name — deliberately NOT
        # wrapper-peeled to `foo` (unlike `env`/`nohup`/`timeout`) so the
        # wrapped command still can't silently inherit `command`'s own
        # blanket allow; the previous behavior (falls through to the default
        # ask, since no rule keys on `command` itself) is unchanged, just
        # given an explicit reason.
        return _ask(
            f"'{display}' runs another command directly, bypassing shell "
            "functions/aliases — the wrapped command isn't visible to this analysis.",
            "unknown",
            shape=shape,
            eligible=True,
        )
    ...
    # to a pure reader cannot mutate anything. A write redirection
    # (`cat > file`, `echo … > file`) is allowed here too, as of this check —
    # `_judge_segment` is only ever reached once this segment's own targets
    # (including its redirection target) are confirmed workspace-confined or
    # under the OS temp directory by the caller's outside-workspace check
    # (see this function's docstring), so writing there is exactly as
    # trusted as `create_file`/`edit_file` on the same path; an escaping
    # write target still asks, via that earlier check, before
    # `_judge_segment` is ever called for the segment. The one case still
    # excluded is a write whose target itself can't be statically resolved
    # (`cat $VAR > $OUT`): the outside-workspace check couldn't have vetted a
    # path it never saw.
    readonly = _READONLY_EXECUTABLES | _READONLY_CMDLETS if windows else _READONLY_EXECUTABLES
    if exe in readonly:
        if segment.writes_file and segment.has_substitution:
            return _ask(
                f"'{display}' contains substitutions whose values cannot be statically resolved.",
                "unknown",
            )
...
def _is_path_offer_eligible(segment: NormalizedSegment) -> bool:
    ...
    """
    eligible = _READONLY_EXECUTABLES | _READONLY_CMDLETS | CD_EXECUTABLES
