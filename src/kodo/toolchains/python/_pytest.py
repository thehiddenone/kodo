...
def parse_pytest_json(report_path: Path) -> ToolchainTestResult:
    """Parse a pytest ``--json-report`` output file into a :class:`ToolchainTestResult`.
    ...
    Returns:
        ToolchainTestResult: Structured test outcome.
    """
    ...
    for test in raw.get("tests", []):
        ...
        cases.append(ToolchainTestCase(name=node_id, passed=passed, message=message))

    return ToolchainTestResult(passed=passed_count, failed=failed_count, cases=cases)


def parse_pytest_stdout(stdout: str, stderr: str) -> ToolchainTestResult:
    """Parse pytest terminal output into a :class:`ToolchainTestResult`.
    ...
    Returns:
        ToolchainTestResult: Best-effort structured outcome; individual case details
    ...
    for line in combined.splitlines():
        ...
        if stripped.startswith("FAILED "):
            ...
            message = stripped
            cases.append(ToolchainTestCase(name=name, passed=False, message=message))
