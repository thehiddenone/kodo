...
class WorkspaceLayout:
    ...
    def etc_dir(self) -> Path:
        ...
        :mod:`kodo.llms.local_registry`, :mod:`kodo.llms.llamacpp._manager`
...
class ProjectLayout:
    ...
    def init_existing(self) -> tuple[bool, bool]:
        ...
        ``.gitignore``) — so pre-existing content is never touched. When
        ``.kodo/`` already exists, ``root`` is already a Kodo project: this
        is a no-op success (nothing on disk is touched) rather than an
        error, so callers like ``scaffold_new_project`` can point at an
        already-scaffolded directory and simply succeed. Otherwise
        ``.kodo/`` and ``kodo.md`` are created; the caller (via
        ``RootMirrorManager.prepare``) follows up with the checkpoint git
        mirror and its mandatory baseline commit.

        Returns:
            tuple[bool, bool]: ``(scaffolded, already_scaffolded)``.
                ``scaffolded`` is ``True`` if the directory was judged empty
                and the standard ``specs/``/``src/``/``test/`` layout was
                created. ``already_scaffolded`` is ``True`` if ``.kodo/``
                was already present and nothing was done; in that case
                ``scaffolded`` is always ``False``.
        ...
        if not self.root.is_dir():
            raise ProjectLayoutError(
                f"{self.root} does not exist or is not a directory — scaffold_new_project "
