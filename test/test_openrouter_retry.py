"""Behavior tests for kodo.llms.openrouter._retry.

OpenRouter's OpenAI-compatible endpoint is reached through the `openai`
Python SDK pointed at a custom base_url, so it raises the same `openai.*`
exception classes the OpenAI/Meta/Google/Alibaba/DeepSeek/Kimi plugins' own
retry modules handle -- this mirrors test_kimi_retry.py's cases exactly
against kodo.llms.openrouter._retry.
"""
...
from kodo.llms.openrouter._retry import (
    RetryExhaustedError,
    UnrecoverableError,
    with_retry,
    with_retry_iter,
...
# depends on this being the SAME class object as every other vendor's.
# ---------------------------------------------------------------------------
