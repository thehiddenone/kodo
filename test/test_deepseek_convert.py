"""Behavior tests for kodo.llms.deepseek._convert.

DeepSeek's OpenAI-compatible endpoint speaks Chat Completions, the same wire
shape as the Google/Alibaba/local-llamacpp plugins -- so these cases mirror
test_alibaba_convert.py's, except there is no ``thought_signature``-style
replay mechanism to test (see kodo/llms/deepseek/_convert.py's module
...
from kodo.llms.deepseek._convert import build_chat_messages, build_openai_tools
