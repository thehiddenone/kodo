"""Behavior tests for kodo.llms.meta._retry.

Meta's Model API is reached through the `openai` Python SDK pointed at a
custom base_url, so it raises the same `openai.*` exception classes the
OpenAI plugin's own retry module handles -- this mirrors
test_openai_retry.py's cases exactly against kodo.llms.meta._retry.
"""
...
from kodo.llms.meta._retry import (
    RetryExhaustedError,
    UnrecoverableError,
    with_retry,
    with_retry_iter,
...
# depends on this being the SAME class object as Anthropic's/OpenAI's.
# ---------------------------------------------------------------------------
