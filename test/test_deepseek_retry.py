"""Behavior tests for kodo.llms.deepseek._retry.

DeepSeek's OpenAI-compatible endpoint is reached through the `openai` Python
SDK pointed at a custom base_url, so it raises the same `openai.*` exception
classes the OpenAI/Meta/Google/Alibaba plugins' own retry modules handle --
this mirrors test_alibaba_retry.py's cases exactly against
kodo.llms.deepseek._retry.
"""
...
from kodo.llms.deepseek._retry import (
    RetryExhaustedError,
    UnrecoverableError,
    with_retry,
    with_retry_iter,
