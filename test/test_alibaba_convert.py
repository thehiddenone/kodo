"""Behavior tests for kodo.llms.alibaba._convert.

Alibaba's OpenAI-compatible endpoint speaks Chat Completions, the same wire
shape as the Google/local-llamacpp plugins -- so these cases mirror
test_google_convert.py's, except there is no ``thought_signature``-style
replay mechanism to test (see kodo/llms/alibaba/_convert.py's module
...
from kodo.llms.alibaba._convert import build_chat_messages, build_openai_tools
