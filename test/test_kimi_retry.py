"""Behavior tests for kodo.llms.kimi._retry.

Kimi's OpenAI-compatible endpoint is reached through the `openai` Python SDK
pointed at a custom base_url, so it raises the same `openai.*` exception
classes the OpenAI/Meta/Google/Alibaba/DeepSeek plugins' own retry modules
handle -- this mirrors test_deepseek_retry.py's cases exactly against
kodo.llms.kimi._retry.
"""
...
from kodo.llms.kimi._retry import (
    RetryExhaustedError,
    UnrecoverableError,
    with_retry,
    with_retry_iter,
