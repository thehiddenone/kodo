"""Behavior tests for kodo.llms.openrouter._convert.

OpenRouter's OpenAI-compatible endpoint speaks Chat Completions, the same
wire shape as the Google/Alibaba/DeepSeek/Kimi/local-llamacpp plugins -- so
these cases mirror test_kimi_convert.py's, since kodo/llms/openrouter/_convert.py
is adapted from kodo/llms/kimi/_convert.py: no ``thought_signature``-style
replay mechanism to test, a ``thinking`` block is asserted dropped outright.
"""
...
from kodo.llms.openrouter._convert import build_chat_messages, build_openai_tools
