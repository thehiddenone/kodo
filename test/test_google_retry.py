"""Behavior tests for kodo.llms.google._retry.

Gemini's OpenAI-compatible endpoint is reached through the `openai` Python
SDK pointed at a custom base_url, so it raises the same `openai.*` exception
classes the OpenAI/Meta plugins' own retry modules handle -- this mirrors
test_openai_retry.py's/test_meta_retry.py's cases exactly against
kodo.llms.google._retry.
"""
...
from kodo.llms.google._retry import (
    RetryExhaustedError,
    UnrecoverableError,
    with_retry,
    with_retry_iter,
